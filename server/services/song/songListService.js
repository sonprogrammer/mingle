  ...
  if (queryValue === "top") {
    ...
    const populatedSongs = await Promise.all(
      songIds.map(async (songId) => {
        const song = await Song.findById(songId).populate("songUploader");
        return song && song.songUploader ? song : null;
      })
    );
    // populatedSongs 배열에서 null 값 제외하고 최대 100개만 추출
    const filteredSongs = populatedSongs
      .filter((song) => song !== null)
      .slice(0, 100);
    return isUserLikedSong.verifyInSong(userId, filteredSongs);
...
async function getUserUploadedSongs(userId, page, pageSize) {
  ...
  const songsInfo = await Promise.all(
    userUploadedSongs.map(async (song) => {
      const likeCount = await songLikedModel.countDocuments({
        songId: song._id,
      });
      return { ...song.toObject(), likeCount };
    })
  );
